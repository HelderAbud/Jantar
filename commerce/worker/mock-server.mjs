...
const PRO_TOKEN = process.env.PRO_TOKEN || 'JL-PRO-DEMO';
...
function fulfill(order) {
  ...
  order.proUrl = `${appBase}?pro=${encodeURIComponent(PRO_TOKEN)}&modo=editor#c=${payload}`;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/checkout') {
      ...
      return json(res, 200, { orderId, checkoutUrl, mock: true, amountCentavos: order.amountCentavos }, origin);
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/webhooks/mercadopago') {
      ...
      if (body?.mock === true && body.orderId) {
        ...
        fulfill(order);
        events.set(key, true);
        return json(res, 200, { ok: true, orderId: order.id, status: order.status, proUrl: order.proUrl }, origin);
      }
